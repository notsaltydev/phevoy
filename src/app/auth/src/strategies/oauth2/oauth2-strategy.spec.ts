...
describe('oauth2-auth-strategy', () => {
    ...
    const successToken = authCreateToken(AuthOAuth2Token, tokenSuccessResponse,
        'strategy') as AuthOAuth2Token;
    const refreshedToken = authCreateToken(AuthOAuth2Token, refreshedTokenPayload,
        'strategy') as AuthOAuth2Token;
    const refreshedTokenWithRefreshToken = authCreateToken(AuthOAuth2Token, refreshedTokenResponse,
        'strategy') as AuthOAuth2Token;
    ...
    beforeEach(() => {
        ...
        TestBed.configureTestingModule({
            ...
            providers: [
                ...
                {provide: WindowRef, useFactory: () => windowMock}, // useValue will clone, we need reference
            ],
        });
    });

    beforeEach(async(inject(
        [OAuth2AuthStrategy, HttpTestingController],
        (_strategy, _httpMock) => {
            strategy = _strategy;
            httpMock = _httpMock;
    ...
    describe('out of the box: type CODE', () => {
            ...
            httpMock.expectOne(
                req => {
                    ...
                    return (req.url === 'http://example.com/token'
                        ...
                        && decodeURIComponent(params['grant_type']) === OAuth2GrantType.AUTHORIZATION_CODE
        ...
        it('handle refresh token with basic client auth', (done: DoneFn) => {
            strategy.setOptions({
                ...
                clientAuthMethod: OAuth2ClientAuthMethod.BASIC,
            });
            ...
            httpMock.expectOne(
                req => {
                    ...
                    return (req.url === 'http://example.com/token'
                        ...
                        && req.headers.get('Content-Type') === 'application/x-www-form-urlencoded'
                        && decodeURIComponent(params['grant_type']) === OAuth2GrantType.REFRESH_TOKEN
        ...
        it('handle refresh token with requestBody client auth', (done: DoneFn) => {
            strategy.setOptions({
                ...basicOptions,
                clientSecret: 'clientSecret',
                clientAuthMethod: OAuth2ClientAuthMethod.REQUEST_BODY,
            });
            strategy.refreshToken(successToken)
                .subscribe((result: AuthResult) => {
                    expect(result).toBeTruthy();
        ...
        it('Handle refresh-token and leaves refresh_token unchanged if present', (done: DoneFn) => {
            ...
            strategy.refreshToken(successToken)
                .subscribe((result: AuthResult) => {
                    ...
                    expect(result.getToken().getOwnerStrategyName()).toEqual(refreshedTokenWithRefreshToken.getOwnerStrategyName());
                    expect(result.getMessages()).toEqual(successMessages);
                    expect(result.getErrors()).toEqual([]); // no error message, response is success
                    expect(result.getRedirect()).toEqual('/');
                    done();
        ...
        it('handle success redirect back with token', (done: DoneFn) => {
            ...
            strategy.authenticate()
                .subscribe((result: AuthResult) => {
                    ...
                    expect(result.getToken().getValue()).toEqual(authCreateToken(AuthOAuth2Token, token, 'strategy').getValue());
                    expect(result.getMessages()).toEqual(successMessages);
                    expect(result.getErrors()).toEqual([]); // no error message, response is success
                    expect(result.getRedirect()).toEqual('/');
                    done();
    ...
    describe('configured: additionnal param: token, grant_type:password', () => {
        ...
        it('handle success login with NO client auth', (done: DoneFn) => {
            const credentials = {email: 'example@phevoy.com', password: '123456'};
            ...
            httpMock.expectOne(
                req => {
                    ...
                    return (req.url === 'http://example.com/token'
                        && req.headers.get('Content-Type') === 'application/x-www-form-urlencoded'
                        && decodeURIComponent(params['grant_type']) === OAuth2GrantType.PASSWORD
